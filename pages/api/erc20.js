...
const schema = Joi.object().keys({
  ...
  amount: Joi.string().required(), // Amount in smallest unit to send
});
...
export default function handler(req, res) {
  ...
  // connect to token
  const contract = new ethers.Contract(value.contract, abi20, wallet);
  // call tranfer method
  contract
    .transfer(value.to, value.amount)
    .then((tx) => {
